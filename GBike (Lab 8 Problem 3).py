...
move_cost = 2
free_shuttle_cost = 0  # No cost for one bike moved from Location 1 to Location 2 by employee
parking_limit = 10  # Max number of bikes at a location before parking cost is incurred
parking_cost = 4  # Cost for exceeding the parking limit at a location
        ...
        for r2 in range(max_r2 + 1):  # Rental at location 2
            ...
            for R1 in range(max_R1 + 1):  # Return at location 1
                for R2 in range(max_R2 + 1):  # Return at location 2
                    ...
                    # Cost calculation: 
                    if a > 0:  # Moving bikes from Location 2 to Location 1
                        move_cost_total = a * move_cost
                    elif a < 0:  # Moving bikes from Location 1 to Location 2
                        move_cost_total = abs(a - 1) * move_cost + free_shuttle_cost  # First bike is free
                    else:
                        move_cost_total = 0

                    # Parking cost: if more than 10 bikes at a location, incur parking cost
                    if new_b1 > parking_limit:
                        move_cost_total += parking_cost
                    if new_b2 > parking_limit:
                        move_cost_total += parking_cost
